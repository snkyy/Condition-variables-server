...
endpoint_t get_macv_id()
{
  struct mproc *rmp;
  endpoint_t macv_ep = 0;
  for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++) {
        if (!(rmp->mp_flags & IN_USE))
                continue;
        if (!strcmp(rmp->mp_name, "macv")) {
                macv_ep = rmp->mp_endpoint;;
                return macv_ep;
        }
  }
}

void tell_macv(m_ptr)
message *m_ptr;
{
  int r;
  endpoint_t macv_ep = -1;
  macv_ep = get_macv_id();
  fprintf(stderr, "hello\n");
  if(macv_ep != -1)
  {
  	r = asynsend3(macv_ep, m_ptr, AMF_NOREPLY);
  }
}
